...
description: >
  ...
  (pink/blue/green/yellow/cream) plus black/white/off-white, Playfair Display 800–900 uppercase display +
  Inter label chrome, square corners, label-pills, tilted decorations, star bursts, stripe
...
typography:
  ...
  card-title:{ fontFamily: "Playfair Display", cqw: 1.15, weight: 700, upper: true, lineHeight: 1.2 }
  label:   { fontFamily: "Inter", px: 13, weight: 600, tracking: "0.08em", upper: true }
  counter: { fontFamily: "Inter", px: 14, weight: 700, tracking: "0.1em", upper: true }
  # — display ramp (Playfair Display 800–900, uppercase, negative tracking) —
  heading-md:{ fontFamily: "Playfair Display", cqw: 2.1, weight: 700, lineHeight: 1.1, tracking: "-0.01em", upper: true }
  quote-text:{ fontFamily: "Playfair Display", cqw: 2.7, weight: 900, lineHeight: 1.15, tracking: "-0.02em", upper: true }
  stat-number:{ fontFamily: "Playfair Display", cqw: 3.3, weight: 900, lineHeight: 1.0 }
  heading-lg:{ fontFamily: "Playfair Display", cqw: 3.3, weight: 800, lineHeight: 1.0, tracking: "-0.02em", upper: true }
  close-title:{ fontFamily: "Playfair Display", cqw: 4.2, weight: 900, lineHeight: 0.95, tracking: "-0.03em", upper: true }
  heading-xl:{ fontFamily: "Playfair Display", cqw: 5.0, weight: 900, lineHeight: 0.95, tracking: "-0.03em", upper: true }
...
components:
  ...
  button-primary:
    ...
    rounded: "0"
    shadow: "0.4cqw 0.4cqw 0 {colors.black}"
    typography: "Playfair Display 700"
    description: "The CTA."
  star-burst:
...
This is the **blockframe** preset remixed onto the **Tu Indemnización Laboral** brand (navy `#0f2a43`
+ dorado `#c9932c`, blanco de tarjeta). The YAML frontmatter above (colors · typography · components) is
...
- **Fonts** — already set to **Playfair Display** (display, tono jurídico serio) / **Inter** (body + chrome/labels); ignore any preset font name lingering in prose.
- **Colors** — use the frontmatter hex. Los "pastel" del preset son aquí **tintas de navy claro**
  sobre las que cicla el fondo; `white` = relleno blanco de tarjeta (legibilidad); `yellow` = el
  **dorado de marca `#c9932c`** (CTA + la única sombra de color). `black` = navy `#0f2a43`, borde y texto.
  Ninguna semántica de color: emparejar por yuxtaposición, no inventar tonos.

- **Tono TIL** — sobrio y de confianza (servicios jurídicos reales). Sin emojis, sin sensacionalismo.
  **Nunca inventar cuantías, plazos ni porcentajes** en los frames (ver "Numerals & Claims"): un dato
  legal mal puesto es un problema, no una errata.
...
The voice is **Playfair Display** at weight 800–900 in tight uppercase with negative tracking (display) +
weight 500 sentence body, and **Inter** weight 600 uppercase 0.08em as the label/chrome
...
- **Playfair Display 800–900 uppercase** negative-tracked display; **Inter** label chrome.
...
- **Squint** — one Playfair Display display moment dominates at 3–6× its neighbor; cards read as a system, not rivals.
...
Two ramps. The **reading/chrome ramp** (Playfair Display body 0.95cqw weight 500, Inter labels in px)
carries copy + chrome; the **display ramp** (Playfair Display `heading-md` 2.1cqw → `heading-xl` 5cqw, weight
...
- **Playfair Display display is uppercase, weight 800–900, negative-tracked** (−0.01..−0.03em); body weight 500 sentence case; **Inter labels uppercase 0.08em**. No sentence-case display, no untracked display.
...
**Ground** `{colors.cream}` (or offwhite) + faint dot-grid. **Composes** label-pill, heading-xl, tilt-deco rect, star-burst, counter. **Focal** a 2–3 line Playfair Display `heading-xl` uppercase, left, under a label-pill; tilted pastel rects + a star burst puncture the right. **Chrome** counter pill. **Accent** the decorations' pastels. **Silence** right third holds decorations. **Fixed** 4px/8px coupling, uppercase display, square. **Free** title, decoration placement/colors. **Density** comfortable.
...
**Ground** `{colors.pink}` (or offwhite). **Composes** label-pill, quote-text in a card-elevated, attribution. **Focal** an Playfair Display `quote-text` (900 uppercase) inside a white bordered+shadowed frame. **Accent** the ground + one decoration. **Silence** moderate. **Fixed** uppercase quote, 4px/8px. **Free** quote, ground. **Density** comfortable.
...
- Set Playfair Display display **uppercase, 800–900, negative-tracked**; open every region with a **label-pill**.
...
- No sentence-case Playfair Display display; no untracked display; no label as plain text.
...
- **Squint** — one Playfair Display display moment dominates; cards read as a system.
...
- **Type** — Playfair Display uppercase 800–900 negative-tracked, fit-to-measure; Inter labels 0.08em; ≥1.4cqw floor.
...
- **Playfair Display + Inter via Google Fonts.** CJK: Noto Sans SC 900 (sentence case — the uppercase signal drops); lean harder on borders/shadows/decoration to carry the brutalist identity.
